...
void StylisedGameCartridge::draw(nxui::Renderer& ren,
    ...
    // V10.27: the default anthracite preset reproduces the V10.25
    // material exactly. Other presets tint only representations that were
    // already cartridges (suspended / launch / lockscreen).
    const bool defaultShell =
        std::abs(panelColor.r - 0.10f) < 0.0005f &&
        std::abs(panelColor.g - 0.11f) < 0.0005f &&
        std::abs(panelColor.b - 0.13f) < 0.0005f;

    nxui::Color frontBody{};
    nxui::Color backBody{};
    nxui::Color sideBase{};
    nxui::Color sideHighlight{};
    ...
    if (defaultShell) {
        ...
        sideHighlight = {
            0.22f, 0.23f, 0.25f, 0.995f * alpha
        };
    } else {
        ...
        auto lit = [](float channel, float light, float floor, float gain) {
            return clamp01(channel * (floor + gain * light) +
                           0.010f * light);
        };

        frontBody = {
            lit(shell.r, frontLight, 0.88f, 0.31f),
            lit(shell.g, frontLight, 0.88f, 0.31f),
            lit(shell.b, frontLight, 0.88f, 0.31f),
            0.995f * alpha
        };
        backBody = {
            lit(shell.r, backLight, 0.76f, 0.24f),
            lit(shell.g, backLight, 0.76f, 0.24f),
            lit(shell.b, backLight, 0.76f, 0.24f),
            0.995f * alpha
        };
        sideBase = {
        ...
        sideHighlight = {
            clamp01(shell.r * 1.45f + 0.08f),
            clamp01(shell.g * 1.45f + 0.08f),
            clamp01(shell.b * 1.45f + 0.09f),
            0.995f * alpha
        };
    }
