...
void CircularSelectionHaloWidget::setTargetRect(const nxui::Rect& target) {
    ...
    const float side = std::max(target.width, target.height) + 14.f;
...
void CircularSelectionHaloWidget::onRender(nxui::Renderer& ren) {
    ...
    ren.drawRoundedRectOutline(
        m_rect.expanded(4.f),
        nxui::Color(1.f, 1.f, 1.f, 0.24f * m_opacity),
        radius + 4.f,
        7.f
    );
    ren.drawRoundedRectOutline(
        m_rect.expanded(1.5f),
        nxui::Color(1.f, 1.f, 1.f, 0.38f * m_opacity),
        radius + 1.5f,
        4.f
    );
    ren.drawRoundedRectOutline(
        m_rect,
        nxui::Color(1.f, 1.f, 1.f, 1.00f * m_opacity),
        radius,
        2.6f
    );
}
